...
func changedSince(db *sql.DB, path string, since, now time.Time) ([]linkChange, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&url, &kind, &isMerged, &closedAt, &checkState, &checkAt, &ghUpdated, &ellyUpdated, &needed, &detail, &commenter); err != nil {
			return nil, err
		}
		switch {
...
// described every completed issue that way.
