...
func TestUpsertAndListOrdering(t *testing.T) {
	...
	s.recordAt("m/reputation", "launch", t0)
	s.recordAt("m/nginx-ingress", "focus", t0.Add(time.Hour))
	s.recordAt("m/reputation", "focus", t0.Add(2*time.Hour)) // newest → first
