...
// A closed issue is how work finishes; a closed pull request is not. Both
// read "closed unmerged" before the kind reached this query.
func TestChangedSinceWordsClosedByKind(t *testing.T) {
	...
	if _, err := s.db.Exec(`update link set closed_at = ?`, now.Add(-time.Hour).UTC().Format(time.RFC3339)); err != nil {
		t.Fatal(err)
	}
	got, err := changedSince(s.db, "m/demo", now.Add(-2*time.Hour), now)
	if err != nil {
		t.Fatal(err)
	}
	var lines []string
