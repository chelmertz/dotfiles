...
d() {
    ...
    code=$(dunstify --appname "Timer" --urgency="$urg" --timeout 1000 --block --replace="$an_id" "$name $sec_string")
    if [ "$code" = 2 ]; then
...
while [[ $seconds -gt 0 ]]; do
    ...
    if [ "$seconds" -eq 0 ]; then
        dunstify --appname "Timer" --replace="$an_id" --urgency=critical "Timer '$name' done, $initial seconds passed" "$(date)"
